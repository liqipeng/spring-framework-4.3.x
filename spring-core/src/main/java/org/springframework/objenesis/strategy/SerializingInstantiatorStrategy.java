 ...
 */
public class SerializingInstantiatorStrategy extends org.springframework.objenesis.strategy.BaseInstantiatorStrategy {
   ...
   public <T> ObjectInstantiator<T> newInstantiatorOf(Class<T> type) {
      ...
      if(JVM_NAME.startsWith(HOTSPOT) || org.springframework.objenesis.strategy.PlatformDescription.isThisJVM(OPENJDK)) {
         // Java 7 GAE was under a security manager so we use a degraded system
         if(isGoogleAppEngine() && org.springframework.objenesis.strategy.PlatformDescription.SPECIFICATION_VERSION.equals("1.7")) {
         ...
         if(org.springframework.objenesis.strategy.PlatformDescription.isAndroidOpenJDK()) {
