...
public class MagicInstantiator<T> implements org.springframework.objenesis.instantiator.ObjectInstantiator<T> {
   ...
   private static final byte[] CONSTRUCTOR_CODE = { org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_aload_0, org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_invokespecial, 0, INDEX_METHODREF_OBJECT_CONSTRUCTOR, org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_return};
   private static final int CONSTRUCTOR_CODE_ATTRIBUTE_LENGTH = 12 + CONSTRUCTOR_CODE.length;

   private static final byte[] NEWINSTANCE_CODE = { org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_new, 0, INDEX_CLASS_TYPE, org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_dup, org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_invokespecial, 0, INDEX_METHODREF_OBJECT_CONSTRUCTOR, org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.OPS_areturn};
    ...
    * which implements {@link org.springframework.objenesis.instantiator.ObjectInstantiator} interface.
   ...
   private <T> org.springframework.objenesis.instantiator.ObjectInstantiator<T> newInstantiatorOf(Class<T> type) {
      String suffix = type.getSimpleName();
      String className = getClass().getName() + "$$$" + suffix;

      Class<org.springframework.objenesis.instantiator.ObjectInstantiator<T>> clazz = org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.getExistingClass(getClass().getClassLoader(), className);

      if(clazz == null) {
         byte[] classBytes = writeExtendingClass(type, className);

         try {
            clazz = org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.defineClass(className, classBytes, getClass().getClassLoader());
         } catch (Exception e) {
   ...
   private byte[] writeExtendingClass(Class<?> type, String className) {
      String clazz = org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.classNameToInternalClassName(className);
      ...
      try {
         ...
         in.write(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.MAGIC);
         in.write(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.VERSION);
         in.writeShort(CONSTANT_POOL_COUNT);

         // set all the constant pool here

         // 1. class
         in.writeByte(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.CONSTANT_Class);
         ...
         in.writeByte(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.CONSTANT_Utf8);
...
//         in.writeUTF("java/lang/Object");
         ...
         in.writeByte(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.CONSTANT_Utf8);
         in.writeUTF(org.springframework.objenesis.instantiator.ObjectInstantiator.class.getName().replace('.', '/'));
         ...
         in.writeByte(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.CONSTANT_Methodref);
         ...
         in.writeByte(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.CONSTANT_NameAndType);
         ...
         in.writeByte(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.CONSTANT_Utf8);
         in.writeUTF(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.classNameToInternalClassName(type.getName()));

         // end of constant pool

         // access flags: We want public, ACC_SUPER is always there
         in.writeShort(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.ACC_PUBLIC | org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.ACC_SUPER | org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.ACC_FINAL);
         ...
         in.writeShort(org.springframework.objenesis.instantiator.util.ClassDefinitionUtils.ACC_PUBLIC);
